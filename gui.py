...
class CEMPoGUI:
    ...
    def show_ptd_edge_info(self):
        # Implementation omitted for brevity, assuming existing logic or can be added if needed
        pass

    def run_ptd_diagnostic(self):
    ...
    def preview_geometry(self): # Renamed from preview_step
    ...
    def build_geometry(self):
        ...
        try:
            if geo_type in ["Analytic Cylinder", "OCC Cylinder (NURBS)"]:
                params.update({'r': self.geo_radius.get(), 'h': self.geo_height.get()})
            elif geo_type == "Analytic Plate":
                params.update({'w': self.geo_width.get(), 'h': self.geo_height.get()})
            elif geo_type == "Analytic Sphere":
                params.update({'r': self.geo_radius.get()})
            elif geo_type == "STEP File":
                if not self.step_file_path: return None
                params.update({
                    'path': self.step_file_path,
                    'unit': self.step_unit_var.get() if hasattr(self, 'step_unit_var') else 'mm',
                    'invert': self.invert_indices_var.get() if hasattr(self, 'invert_indices_var') else ''
                })
            if self.current_geo_params == params and self.current_geometry is not None:
        ...
        try:
            ...
            if geom:
                ...
                self.current_geo_params = params
                self.cached_mesh_data = None
                self.cached_mesh_params = {}
            return geom
        except Exception as e:
    ...
    def plot_multi_surface_mesh(self, surfaces, freq, samples):
        try:
            ...
            total_points = 0
            n_total = len(surfaces)
            current_params = {'freq': freq, 'samples': samples, 'surfaces_id': id(surfaces)}
            for i, surf in enumerate(surfaces):
    ...
    def run_calculation(self):
        ...
        if 'discrete_po' in current_algo_id and self.cached_mesh_data is not None:
            check_params = {'freq': freq, 'samples': samples, 'surfaces_id': id(geo)}
            if check_params == self.cached_mesh_params:
                cached_mesh = self.cached_mesh_data
                self.log("💡 Optimization: Using existing mesh from preview.")
        self.progress_var.set(0)
    ...
    def _get_geometry_params(self):
        ...
        try:
            if "Cylinder" in geo_type:
                ...
                params['radius'] = self.geo_radius.get()
            elif "Wedge" in geo_type: # Add Wedge params
                params['length'] = self.geo_length.get()
                params['width'] = self.geo_width.get()
                params['height'] = self.geo_height.get()
        except: pass
        return params
    ...
    def export_to_csv(self):
        ...
        else: geo_label = geo_type
        default_filename = f"rcs_{mode}_{geo_label}_{freq_mhz:.1f}MHz.csv"
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV Files", "*.csv"), ("All Files", "*.*")], initialfile=default_filename, title="选择导出位置")
        if not file_path: return
        try:
            import pandas as pd
            if mode == '2d':
                ...
                for i, t in enumerate(theta_deg):
                    for j, p in enumerate(phi_deg):
                        data_list.append({'Theta': t, 'Phi': p, 'RCS(dBsm)': rcs_2d[i, j]})
    ...
    def save_config(self):
        ...
        try:
            ...
            config['geo_params_cache'] = self.geo_params_cache
            if self.step_file_path:
            ...
            # PTD
            config['ptd_enabled'] = self.ptd_enabled_var.get()
            config['ptd_edges'] = self.ptd_edges_var.get()
            try: config['polarization'] = self.pol_var.get()
            except: pass
    ...
    def load_config(self):
        ...
        try:
            ...
            if 'algorithm' in config:
                ...
                if algo_id == 'discrete_po': algo_id = 'discrete_po_sinc_dual'
            ...
            set_var(self.ptd_enabled_var, 'ptd_enabled')
            set_var(self.ptd_edges_var, 'ptd_edges')
            set_var(self.pol_var, 'polarization')
            try:
                if self.ptd_enabled_var.get():
                     self.ptd_entry.configure(state='normal')
            except: pass
