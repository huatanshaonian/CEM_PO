...
### v1.2.1 (2026-01-29)
*   **🐛 修复重大 Bug**: 修复了条带网格（退化网格）在点击计算时无法正确匹配缓存，导致几何预计算被错误触发的问题。现在计算流程实现了真正的“零冗余”。
*   **💾 增强配置保存**: `config.json` 现在会完整保存算法选择、退化网格开关、PTD 设置及几何参数。

### v1.2 (2026-01-29)
*   **✨ 新增特性**: 正式支持退化曲面（如球极点）的**条带网格 (Strip Mesh)** 生成与计算，显著提升了极点处的物理正确性。
*   **🚀 性能优化**: GPU 模式下支持退化网格与普通网格的混合批量计算。
